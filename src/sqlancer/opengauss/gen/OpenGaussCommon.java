...
public final class OpenGaussCommon {

    private OpenGaussCommon() {
    }
    ...
    public static void addCommonExpressionErrors(ExpectedErrors errors) {
        ...
        errors.add("NULL input for detoast datum");
        errors.add("could not determine which collation to use");
        errors.add("could not determine polymorphic type because input has type");
        errors.add("The input NUMCacheEntry is invalid, which is Null");
        errors.add("interval units");
        errors.add("WITH CHECK OPTION is supported only on auto-updatable views");
        ...
        errors.add("cannot alter type of column");
    ...
    public static boolean appendDataType(OpenGaussDataType type, StringBuilder sb, boolean allowSerial,
    ...
    private enum StorageParameters {
        ...
        // toast_tuple_target
        PARALLEL_WORKERS("parallel_workers", (r) -> r.getInteger(1, 32)),
    ...
    public static void generateWith(StringBuilder sb, OpenGaussGlobalState globalState, ExpectedErrors errors) {
        if (Randomly.getBoolean()) {
            sb.append(" WITH (");
    ...
    public static void addTableConstraints(boolean excludePrimaryKey, StringBuilder sb, OpenGaussTable table,
            OpenGaussGlobalState globalState, ExpectedErrors errors) {
    ...
    public static void addTableConstraint(StringBuilder sb, OpenGaussTable table, OpenGaussGlobalState globalState,
            ExpectedErrors errors) {
        addTableConstraint(sb, table, globalState, Randomly.fromOptions(TableConstraints.values()), errors);
    }

    private static void addTableConstraint(StringBuilder sb, OpenGaussTable table, OpenGaussGlobalState globalState,
            TableConstraints t, ExpectedErrors errors) {
        List<OpenGaussColumn> randomNonEmptyColumnSubset = table.getRandomNonEmptyColumnSubset();
        List<OpenGaussColumn> otherColumns;
        OpenGaussCommon.addCommonExpressionErrors(errors);
        switch (t) {
        case CHECK:
            sb.append("CHECK(");
            sb.append(OpenGaussVisitor.getExpressionAsString(globalState, OpenGaussDataType.BOOLEAN, table.getColumns()));
            sb.append(")");
        ...
        case FOREIGN_KEY:
            ...
            OpenGaussTable randomOtherTable = globalState.getSchema().getRandomTable(tab -> !tab.isView());
    ...
    private static void appendIndexParameters(StringBuilder sb, OpenGaussGlobalState globalState,
            ExpectedErrors errors) {
        if (Randomly.getBoolean()) {
    ...
    // complete
    private static void appendExcludeElement(StringBuilder sb, OpenGaussGlobalState globalState,
        ...
        if (Randomly.getBoolean()) {
            ...
            // append expression
            sb.append("(");
            sb.append(OpenGaussVisitor.asString(OpenGaussExpressionGenerator.generateExpression(globalState, columns)));
            sb.append(")");
        }
        if (Randomly.getBoolean()) {
            sb.append(" ");
